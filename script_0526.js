...
function findCloserTextIndex(imageEmbedding, textEmbedding0, textEmbedding1) {
    ...
    console.debug(distance0);
    console.debug(distance1);
...
async function updateTextOptions() {
    ...
    const imageEmbedding = imageEmbeddings[currentImageIndex].embedding;
    const textEmbedding0 = textEmbeddings[randomOptionIndex].embedding;
    const textEmbedding1 = textEmbeddings[anotherRandomOptionIndex].embedding;

    var answers = findCloserTextIndex(imageEmbedding, textEmbedding0, textEmbedding1);
    var answerNum = answers[0];
    var distance0 = answers[1];
    var distance1 = answers[2];
...
function checkAnswer() {
    ...
    if (selectedAnswer) {
        ...
        if (selectedAnswer.value === 'correct') {
            ...
            randomIndex = Math.floor(Math.random() * 205); // generates a random number between 0 and 204
            randomImage = 'outimgs/' + randomIndex + '.jpg'; // constructs the path to the random image
            quizImage.src = randomImage;
            updateTextOptions();
